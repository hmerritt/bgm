...
fn main() {
    ...
    println!("cargo:rerun-if-changed=assets/menu-swap.png");
...
fn generate_windows_resources(manifest_dir: &Path, out_dir: &Path) {
    ...
    let swap_source_png = manifest_dir.join("assets").join("menu-swap.png");
    if !swap_source_png.exists() {
        panic!(
            "missing source menu swap image: {}",
            swap_source_png.display()
        );
    }
    ...
    let generated_swap_bmp = out_dir.join("menu-swap.bmp");
    generate_menu_bitmap(&swap_source_png, &generated_swap_bmp);
    ...
    let generated_swap_ico = out_dir.join("menu-swap.ico");
    generate_menu_icon(&swap_source_png, &generated_swap_ico);
    ...
    let swap_bmp_path_for_rc = generated_swap_bmp.to_string_lossy().replace('\\', "/");
    ...
    let swap_ico_path_for_rc = generated_swap_ico.to_string_lossy().replace('\\', "/");
