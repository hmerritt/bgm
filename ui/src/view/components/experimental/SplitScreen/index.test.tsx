...
describe("SplitScreen component", () => {
	test("renders side-by-side by default, allows sx overrides, and does not inject pane chrome", async () => {
		...
		expect(screen.queryByTestId("split-screen-sector-topLeft")).not.toBeInTheDocument();
		expect(getComputedStyle(root).minHeight).toBe("420px");
		expect(screen.queryByTestId("split-screen-pane-handle-a")).not.toBeInTheDocument();
	...
	test("switches to quad mode and renders horizontal divider", async () => {
		...
		await waitFor(() => {
			expect(screen.getByTestId("split-screen-divider-horizontal")).toBeInTheDocument();
			expect(screen.getByTestId("split-screen-sector-topLeft")).toBeInTheDocument();
			expect(screen.getByTestId("split-screen-sector-bottomRight")).toBeInTheDocument();
		});
	});
	...
	test("renders no placeholder UI for empty sectors", async () => {
		...
		await renderBasic(
			<SplitScreen panes={[{ id: "a", node: <div data-testid="pane-a-content">A</div> }]} />
		);
